...
protocol KPDatePickerDelegate: class {
...
class KPDatePicker: UIView {
    ...
    struct Config {
        ...
        var contentBackgroundColor: UIColor = UIColor.yellow
        var headerBackgroundColor: UIColor = UIColor.green
        var btnDoneColor: UIColor = UIColor.white
        var btnCancelColor: UIColor = UIColor.black
        
        var overlayBackgroundColor: UIColor = UIColor.cyan.withAlphaComponent(0.4)
    ...
    weak var delegate: KPDatePickerDelegate?
    ...
    // MARK: - Init
    static func getFromNib() -> KPDatePicker {
        return UINib.init(nibName: String(describing: self), bundle: nil).instantiate(withOwner: self, options: nil).last as! KPDatePicker
    }
    ...
    fileprivate func setup(_ parentVC: UIViewController) {
        ...
        btnCancel.setTitle(config.btnCancelTitle, for: UIControlState())
        
        btnDone.setTitleColor(config.btnDoneColor, for: UIControlState())
        btnCancel.setTitleColor(config.btnCancelColor, for: UIControlState())
